...
export function useTranslations(locale: Locale) {
  return (key: UIKey): string => {
    ...
    return (
      table[key] ?? (ui[defaultLocale] as Record<string, string>)[key] ?? key
    );
  };
}
...
/** Human-readable date in the active locale, e.g. "12 Μαρτίου 2025" / "12 March 2025". */
export function formatDate(date: Date, locale: Locale): string {
  return date.toLocaleDateString(locale === "en" ? "en-GB" : "el-GR", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
